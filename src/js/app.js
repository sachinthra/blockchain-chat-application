...
App = {
  ...
  initWeb3: function () {
    ...
    web3.eth.getAccounts(function (err, accs) {
      if (err != null) {
        alert(
          "There was an error fetching your account, please try again later."
        );
        return;
      }
      account = accs[0];
      if (!account) {
        ...
        alert(
          "Could not fetch your account. Make sure you are logged in to MetaMask, then refresh the page."
        );
        return;
      }
  ...
  getContractProperties: function () {
    var self = this;
    var meta;
    App.contracts.ChatWei.deployed()
      ...
      .then(function (value) {
        ...
        for (i = 0; i < numRegisteredUsers; i++) {
          select +=
            "<option val=" + i + ">" + registeredUsersAddress[i] + "</option>";
        }
      ...
      if (err === null) {
        ...
        web3.eth.getBalance(account[0], function (err, balance) {
          if (err === null) {
            if (balance == 0) {
              alert(
                "Your account has zero balance. You must transfer some Ether to your MetaMask account to be able to send messages with ChatWei. Just come back and refresh this page once you have transferred some funds."
              );
              App.setStatus("Please buy more Ether");
              return;
            } else {
              document.getElementById("myBalance").innerHTML =
                web3.fromWei(balance, "ether").toNumber() + " Ether";
  ...
  registerUser: function () {
    ...
    App.contracts.ChatWei.deployed()
      ...
      .then(function (result) {
        ...
        self.setStatus(
          "User is registered...gas spent: " + gasUsedWei + "(Wei)"
        );
        alert(
  ...
  sendMessage: function () {
    ...
    App.contracts.ChatWei.deployed()
      ...
      .then(function (result) {
        ...
        var gasUsedEther = web3.fromWei(gasUsedWei, "ether");
        self.setStatus(
          "Message successfully sent...gas spent: " + gasUsedWei + " Wei"
        );
