...
contract ChatWei {
    // users transmit "Message" objects that contain the content and data of the message.
    struct Message {
        address sender;
        bytes32 content;
        uint256 timestamp;
    ...
    // the chat owner and all the user details
    ...
    struct Inbox {
        uint256 numSentMessages;
        uint256 numReceivedMessages;
        mapping(uint256 => Message) sentMessages;
        mapping(uint256 => Message) receivedMessages;
    }
    ...
    {
        return (
            contractProperties.ChatWeiOwner,
        ...
        )
    {
        ...
        uint256[] memory timestamp = new uint256[](16);
